...
async function handleSubmit() {
  const ok = await auth.loginAdmin(form.value)
...
<template>
  <main class="flex min-h-screen items-center justify-center bg-bg px-6">
    <form class="card-panel w-full max-w-sm p-8" @submit.prevent="handleSubmit">
      <span class="label-eyebrow">Admin Area</span>
      <h1 class="mt-2 font-display text-2xl text-ink">Masuk Admin</h1>
      <p class="mt-1 text-sm text-muted">Khusus untuk pengelola Toko Sparepart Vespa.</p>
      ...
      <button class="btn-primary mt-6 w-full" type="submit" :disabled="auth.loading">
        {{ auth.loading ? 'Memproses...' : 'Masuk sebagai Admin' }}
      </button>
    </form>
  </main>
</template>
